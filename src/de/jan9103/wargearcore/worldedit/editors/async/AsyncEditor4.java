...
@Deprecated public abstract class AsyncEditor4 extends StackWeAction implements Runnable {
	...
	public AsyncEditor4(World we,BigArea ar,boolean upd,WeSender se){
		super(se);
		u   =upd; w=we;
	...
	@Override public void run(){//TODO dosnt paste @a -- hightchunks?!
		if(TPS.tps<5&&goon){
			Bukkit.getScheduler().runTaskLater(WGC.wgc,this,3); return;
		}
		final Long stop=System.nanoTime()+(50000000+TPS.d0>>1);//50000000 = 1tick

		//TPS.nt();//TODO calc tick end
		while(System.nanoTime()<stop&&goon){
			ChunkLoader.add(chun);
			System.out.println(chun.getX()+" "+chun.getZ());
			cxl=cxl<0?16+(cxl%16):cxl%16;
			cxh=cxh<0?16+(cxh%16):cxh%16;
			czl=czl<0?16+(czl%16):czl%16;
			czh=czh<0?16+(czh%16):czh%16;
